...
export default function Jobs({ projectId, jobs, onRefresh }) {
  ...
  const removeJob = async (jobId) => {
    ...
    try {
      ...
      if (output?.jobId === jobId) setOutput(null)
      onRefresh()
    } finally {
      setDeleting(null)
    }
  ...
  const cleanUpCompleted = async () => {
    setCleaningUp(true)
    try {
      const toRemove = (jobs || []).filter((j) => ['completed', 'failed'].includes(j.status))
      for (const j of toRemove) {
        await api.jobs.delete(projectId, j.id)
      }
      if (toRemove.length) onRefresh()
    } finally {
      setCleaningUp(false)
    }
  }
  ...
  const showOutput = async (job) => {
    if (output?.jobId === job.id) {
      setOutput(null)
      setScreenshots([])
      setPreviewUrl(null)
      return
    }
    setLoadingOutput(job.id)
    try {
      ...
      if (job.runner_key === 'web_gowitness') {
        const list = await api.screenshots.list(projectId)
  ...
  const outputJob = output ? jobs.find((j) => j.id === output.jobId) : null
  const isOutputJobRunning = outputJob && (outputJob.status === 'running' || outputJob.status === 'pending')

  useEffect(() => {
    if (!output?.jobId || !isOutputJobRunning) return
    const interval = setInterval(async () => {
      try {
        const text = await api.jobs.output(projectId, output.jobId)
  ...
  const completedOrFailed = (jobs || []).filter((j) => ['completed', 'failed'].includes(j.status)).length

  if (!jobs?.length) return <div style={styles.empty}>No scan jobs yet. Run a checklist item from the Checklist tab.</div>
        ...
        {jobs.map((j) => (
          <li key={j.id} style={styles.item}>
            <div style={styles.row}>
              ...
              <span style={{ ...styles.status, ...statusStyle(j.status) }}>{j.status}</span>
            ...
            {output?.jobId === j.id && (
              <>
                {isOutputJobRunning && (
                  <div style={styles.liveBadge}>Live output — updating every 1.5s</div>
                )}
            ...
            {j.error_message && (
              <div style={styles.err}>{j.error_message}</div>
            )}
          </li>
        ))}
      </ul>
...
const styles = {
  ...
  status: { textTransform: 'capitalize', fontSize: '0.9rem' },
  time: { color: 'var(--text-muted)', fontSize: '0.85rem' },
  btn: { marginLeft: 'auto', background: 'var(--surface)', color: 'var(--accent)', border: '1px solid var(--border)' },
  stopBtn: { background: 'var(--danger)', color: 'var(--accent-text)' },
  ...
  liveBadge: { fontSize: '0.8rem', color: 'var(--warn)', marginTop: '0.25rem' },
  pre: { marginTop: '0.5rem', padding: '0.75rem', background: 'var(--bg)', borderRadius: 'var(--radius)', fontSize: '0.8rem', overflow: 'auto', maxHeight: 400 },
  err: { marginTop: '0.25rem', color: 'var(--danger)', fontSize: '0.85rem' },
